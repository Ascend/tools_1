...
class Graph(object):
    def __init__(self, debug_id=Constant.DEFAULT_DEBUG_ID):
        ...
        self.graph_all = os.path.join(self.graph_root, Constant.GRAPH_ALL)
        self.graph_build = os.path.join(self.graph_root, Constant.GRAPH_BUILD)
    ...
    def _init_dirs(self):
        """Create graph dirs."""
        util.create_dir(self.graph_all)
        util.create_dir(self.graph_build)

    def _prepare_npu_graphs(self):
        """Copy ge graphs to graph dir. """
        # move graphs to precision data dir
        graph_files = util.list_ge_graph_files(self.graph_all)
        ...
        if len(self.build_files) == 0:
            self.log.warning("Can not find any build files in dir: %s", self.graph_all)
    ...
    def _get_ops_nums_in_graph(self, graph_name):
        count = 0
        graph_path = os.path.join(self.graph_all, graph_name)
