...
function node_common_train()
{
    ...
    for((i=0;i<${DEVICE_NUM};i++));do
    {
        ...
        # clear and create path. it's  no need to delete and recreate RUN_PATH. because one train will be failed when
        # two training action are exacuted almostly at the same time in container under the premise of calculation force segmentation
