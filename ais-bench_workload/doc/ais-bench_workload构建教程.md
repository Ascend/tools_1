...
| 参数                    | {stubs_file}                                                 | {mode}                                                       | {secondary-folder-name}                                      | {third-folder-name}                                          | {version}                                                    | {type}                                    |
| ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ----------------------------------------- |
| 参数说明                | 选择stubs基础工具包，即选择构建测试软件包使用的aarch64和x86_64平台 | 选择构建测试软件包的适用场景。取值为：train(训练场景)、inference(推理场景) | 二级子目录名称，对应ais-bench_workload/src目录下二级子目录名称 | 三级子目录名称，对应ais-bench_workload/src目录下三级子目录名称 | 模型框架版本号，仅{mode}配置为train时支持                    | 线上或离线环境，仅{mode}配置为train时支持 |
| bert推理                | Ais-Benchmark-Stubs-aarch64-1.0.tar.gz<br/>Ais-Benchmark-Stubs-x86_64-1.0.tar.gz<br/>二选一 | inference                                                    | language                                                     | bert                                                         | NA                                                           | NA                                        |
| widedeep推理            | 同上                                                         | inference                                                    | recommendation                                               | widedeep                                                     | NA                                                           | NA                                        |
| 图像分类推理            | 同上                                                         | inference                                                    | vision                                                       | classification_and_detection                                 | NA                                                           | NA                                        |
| huawei/bert训练         | 同上                                                         | train                                                        | huawei                                                       | train_mindspore_bert                                         | 线上训练时可取值：r1.3、r1.5、r1.7<br>线下训练时可取值：r1.5、r1.6、r1.7、r1.8、r1.9 | modelarts或无                             |
| huawei/resnet训练       | 同上                                                         | train                                                        | huawei                                                       | train_mindspore_resnet                                       | 线上训练时可取值：r1.3、r1.5、r1.7<br/>线下训练时可取值：r1.5、r1.6、r1.7、r1.8、r1.9 | modelarts或无                             |
| huawei/deeplabv3        | 同上                                                         | train                                                        | huawei                                                       | train_mindspore_deeplabv3                                    | 线下训练时可取值：r1.9                                       | NA                                        |
| huawei/faster_rcnn      | 同上                                                         | train                                                        | huawei                                                       | train_mindspore_faster_rcnn                                  | 线下训练时可取值：r1.9                                       | NA                                        |
| huawei/widedeep训练训练 | 同上                                                         | train                                                        | huawei                                                       | train_mindspore_widedeep                                     | 线下训练时可取值：r1.9                                       | NA                                        |
| huawei/其它模型训练     | 同上                                                         | train                                                        | huawei                                                       | train_mindspore_deepspeech2<br>train-mindspore_pangu_alpha<br>train_tensorflow_bert_base<br>train_tensorflow_densenet121<br>train_tensorflow_mobileneetv2<br>train_tensorflow_nezha_large<br>train_tensorflow_resnet50<br>train_tensorflow_resnet101<br>train_tensorflow_resnext50<br>train_tensorflow_ssd_resnet34<br>train_tensorflow_vgg16<br>train_tensorflow_yolov3 | master                                                       | NA                                        |
| 英伟达模型训练          | 同上                                                         | train                                                        | nvidia                                                       | train_tensorflow_bert<br>train_tensorflow_resnet             | NA                                                           | NA                                        |
