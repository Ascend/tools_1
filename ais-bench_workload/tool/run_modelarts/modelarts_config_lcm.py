...
access_config = ed({
    # 登录需要的ak sk信息
    'access_key': 'DTWSUE8JEWSNCJOQPR3V',
    'secret_access_key': 'bzb671j5dzZVZO8TIVPBF1rPU19JVPZl8FaLO2QN',
...
session_config = ed({
    ...
    'hyperparameters': [
        ...
        # 不需要修改
        {'label': 'data_sink_steps', 'value': '10'},
        # 不需要修改
        {'label': 'accumulation_steps', 'value': '1'},
        # 保存ckpt的step数 注意 该值必须要跟step数保存一致 这样提高性能
        {'label': 'save_checkpoint_steps', 'value': '10000'},
        # 保存ckpt的个数 默认为1 不需要修改
        {'label': 'save_checkpoint_num', 'value': '1'},

        {'label': 'bert_network', 'value': 'large'},
        
        {'label': 'enable_profiling', 'value': 'True'},

        {'label': 'large_batch_size', 'value': '80'},
        {'label': 'seq_length', 'value': '256'},
        {'label': 'warmup_steps_', 'value': '16900'},
        {'label': 'power', 'value': '2'},
        {'label': 'data_type', 'value': 'md'},
        {'label': 'train_with_eval', 'value': 'false'},
        {'label': 'end_learning_rate', 'value': '0.00008'},
    ],
    # 输入数据集obs目录,请按样例格式填写
    'inputs': '/pcl-xufan/prot_bert/md_data/uniref90_seq256_train/1000/',
    #'inputs': '/mlperf/dataset/en-wiki-512-20files/',
    # obs代码路径 程序会自动拷贝到该路径
    'code_dir': '/0923/00lcm/hzh_bert_full/',
    # 启动文件 必须要在code_dir路径下，请按样例格式填写
    'boot_file': '/0923/00lcm/hzh_bert_full/run_pretrain.py',

    # 如下为运行相关参数
    # job名称  如果云环境Modelarts服务训练作业job队列中没有，则会新建一个job；若和已有job同名，则会在该job中，新建测试实例.
    'job_name': "hzh_bert_full",

    # 使用容器类型与镜像版本
    'framework_type': 'Ascend-Powered-Engine',
    'framework_version': 'MindSpore-1.7-c81-python3.7-euleros2.8-aarch64',
    ...
    "out_base_url": "/0923/res/bert_full_train/",
    # job 描述前缀
    "job_description_prefix": 'bert_full_train from sdk',
})
...
session_config_v2 = ed({
    ...
    # 输入数据集obs目录,请按样例格式填写
    'inputs': '/mlperf/dataset/en-wiki-512/',
    # obs代码路径 程序会自动拷贝到该路径. 和boot_files一起用于复合参数 training_files
    'code_dir': '/0923/lihui/code/bert/',
    # 启动文件 必须要在code_dir路径下，请按样例格式填写
    'boot_file': '/0923/lihui/code/bert/run_pretrain.py',
    ...
    # 使用容器类型与镜像版本
    'framework_type': 'Ascend-Powered-Engine',
    'framework_version': 'mindspore_1.7.0-cann_5.1.0-py_3.7-euler_2.8.3-aarch64',
    ...
    # 输出信息基准路径 整体路径为 train_url = out_base_url/version_name
    "out_base_url": "/0923/lihui/result/",
    # job 描述前缀
