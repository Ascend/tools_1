...
class modelarts_handler():
    ...
    def sync_job_log(self, session_config):
        ...
        if not os.path.exists(dstpath):
            print("dstpath:{} not exist no get log".format(dstpath))
            return
    ...
    def wait_for_job(self, job_instance, session_config, timeout):
        count = 0
        while True:
            time.sleep(10)
            count = count + 1
            if timeout != 0 and count*10 > timeout:
                print("job stop timeout:{} count:{} match".format(timeout, count*1))
                job_instance.stop_job_version()
            if count % 10 == 0:
    ...
    def run_job(self, session_config, localpath, timeout=0):
        ...
        self.wait_for_job(job_instance, session_config, timeout)